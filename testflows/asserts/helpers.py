...
def snapshot(value, id, output=None, path=None, encoder=repr):
    ...
    class SnapshotError(object):
        def __init__(self, snapshot_value, actual_value, diff=None):
        ...
        def __repr__(self):
            ...
            r += "\"\"\", diff=\"\"\"\n"
            r += '\n'.join([line.strip("\n") for line in difflib.ndiff(self.snapshot_value.splitlines(), self.actual_value.splitlines())])
            r += "\n\"\"\")"
            return r
    try:
    ...
    if os.path.exists(filename):
        with open(filename, "r") as fd:
            ...
            if not (snapshot_value == repr_value):
                return SnapshotError(snapshot_value, repr_value)
            return True
    else:
